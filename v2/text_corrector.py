...
class ONNXManager(QObject):
        ...
        try:
            ...
            try:
                if is_seq2seq:
                    ...
                    model = ORTModelForSeq2SeqLM.from_pretrained(self.model_dir, provider=providers[0], use_cache=False)
                    self.pipeline = pipeline("text2text-generation", model=model, tokenizer=tokenizer)
                else:
                    from optimum.onnxruntime import ORTModelForCausalLM
                    model = ORTModelForCausalLM.from_pretrained(self.model_dir, provider=providers[0], use_cache=False)
                    self.pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer)
            except Exception as cuda_error:
                # CUDA failed, retry with CPU only
                if cuda_available:
                    log_debug(f"CUDA execution failed: {cuda_error}, falling back to CPU")
            ...
            self.is_loaded = True
            self.loading = False
            self.status_changed.emit("ONNX Ready", "#4CAF50")
            return True
            
        except Exception as e:
            self.is_loaded = False
            self.loading = False
            self.status_changed.emit("ONNX Error", "red")
            log_debug(f"ONNX Load Error: {e}")
            return False
    ...
    def proofread(self, text):
        ...
        try:
            if self.is_seq2seq:
                # T5 / BART style seq2seq expects instructions to be baked in or just correct grammar directly
                prompt = f"Fix grammar: {text}"
                log_debug(f"[ONNX] Using Seq2Seq. Prompt: {prompt[:100]}...")
                result = self.pipeline(prompt, max_new_tokens=200, truncation=True)[0]['generated_text']
            else:
                prompt = f"Fix grammar and typos in the following text:\n{text}\n\nCorrected:"
                log_debug(f"[ONNX] Using Causal LM. Prompt: {prompt[:100]}...")
                # For causal LMs, pipeline returns the entire text including prompt if return_full_text is True
                out = self.pipeline(prompt, max_new_tokens=200, return_full_text=False, truncation=True)
    ...
    def chat(self, messages, max_tokens=1000):
        ...
        It uses the same prompt engineering as proofread() but adapted for chat context.
        ...
        try:
            # Extract the latest user message and any relevant context
            # For ONNX, we'll use a simple approach: combine recent context + latest request
            user_content = ""
            for msg in reversed(messages[-4:]):  # Last 4 messages for context
            ...
            if len(user_content) > 1000:
                user_content = user_content[-1000:]  # Keep last 1000 chars
            
            if self.is_seq2seq:
                prompt = f"Refine text: {user_content}"
                log_debug(f"[ONNX Chat] Using Seq2Seq. Prompt: {prompt[:100]}...")
                result = self.pipeline(prompt, max_new_tokens=max_tokens, truncation=True)[0]['generated_text']
            else:
                prompt = f"Refine and improve the following text:\n{user_content}\n\nImproved:"
                log_debug(f"[ONNX Chat] Using Causal LM. Prompt: {prompt[:100]}...")
                out = self.pipeline(prompt, max_new_tokens=max_tokens, return_full_text=False, truncation=True)
                result = out[0]['generated_text']
