...
class ModelManager(QObject):
    ...
    def correct_text(self, text, custom_instruction=None):
        ...
        try:
            ...
            corrected = strip_meta_commentary(corrected)
    ...
    def _show_diff(self, original, corrected):
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "equal":
                ...
                # Changed words — highlight in green
                changed = " ".join(corr_words[j1:j2])
                html_parts.append(
                    f'<span style="background-color: rgba(74, 222, 128, 0.2); '
                    f'color: #86efac; border-radius: 3px; padding: 1px 2px;">{changed}</span>'
                )
            elif tag == "insert":
                # Inserted words — highlight in green
                inserted = " ".join(corr_words[j1:j2])
                html_parts.append(
                    f'<span style="background-color: rgba(74, 222, 128, 0.2); '
                    f'color: #86efac; border-radius: 3px; padding: 1px 2px;">{inserted}</span>'
                )
            # tag == 'delete' — words removed, skip

        html = " ".join(html_parts)
        self.corrected_edit.setHtml(
            f'<div style="color: #e2e8f0; font-size: 14px; font-family: Segoe UI;">{html}</div>'
        )
